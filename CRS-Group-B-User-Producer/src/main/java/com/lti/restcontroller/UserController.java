...
public class UserController {
	...
	public ResponseEntity<?> login(@RequestBody User user) throws StudentNotFoundException {
		...
		// try {
		// userService.verifyCredetials(user.getUsername(), user.getPassword(),
		// user.getRole());
		// } catch (StudentNotFoundException e) {
		// return new ResponseEntity(e.getMessage() + e.getStudentID(),
	...
	public ResponseEntity<?> studentRegistration(@RequestBody Student student)
			throws UsernameUsedException, StudentNotFoundException {
		...
		// return new ResponseEntity(e.getMessage() + e.getUsername(),
	...
	@RequestMapping(value = "/logClientData", consumes = MediaType.APPLICATION_JSON, method = RequestMethod.POST)
	public ResponseEntity<?> logClientData(@RequestBody ClientDetail clientData) {
		try {
			File file = new File("CRS-Group-B-User-Producer\\src\\main\\resources\\clientData.txt");
			FileWriter writer = new FileWriter(file);
			writer.write(clientData.getClientData());
			writer.close();
			return new ResponseEntity<>(HttpStatus.OK);
		} catch (IOException e) {
			e.printStackTrace();
		}
		return new ResponseEntity<>("Not currently working", HttpStatus.NOT_ACCEPTABLE);
	}
}
